  ...
  apiRouter.get("/subjects", async (req, res) => {
    try {
      ...
      if (isNaN(branchId) || isNaN(semesterNumber)) {
        return res.status(400).json({ message: "Invalid branch ID or semester number" });
      }
      
      const subjects = await storage.getSubjectsByBranchAndSemester(branchId, semesterNumber);
      res.json(subjects);
    } catch (error) {
